		...
		<div class="btn-group btn-group-sm" component="composer/submit/container">
			...
			<div component="composer/submit/options/container" data-submit-options="{submitOptions.length}" class="btn-group btn-group-sm {{{ if !(submitOptions.length || canSchedule) }}}hidden{{{ end }}}">
				...
				<ul class="dropdown-menu dropdown-menu-end p-1" role="menu">
					<li><a class="dropdown-item rounded-1 display-scheduler {{{ if !canSchedule }}}hidden{{{ end }}}" role="menuitem">{{tx("topic:composer.post-later")}}</a></li>
					<li><a class="dropdown-item rounded-1 cancel-scheduling hidden" role="menuitem">{{tx("modules:composer.cancel-scheduling")}}</a></li>
					{{{ each submitOptions }}}
					<li><a class="dropdown-item rounded-1" href="#" data-action="{./action}" role="menuitem">{{{if ./icon }}}<i class="{{./icon}}"></i> {{{ end }}}{{tx(./text)}}</a></li>
					{{{ end }}}
				</ul>
			</div>
		</div>
