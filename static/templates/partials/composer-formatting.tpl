<div class="d-flex justify-content-between gap-2 align-items-center formatting-bar m-0">
	<ul class="list-unstyled mb-0 d-flex formatting-group gap-2 overflow-auto">
		{{{ each formatting }}}
			...
			<li class="dropdown bottom-sheet" title="{{tx(./title)}}">
		...
		<li title="{{tx("modules:composer.upload-picture")}}">
			<button data-format="picture" class="img-upload-btn btn btn-sm btn-link text-reset" aria-label="{{tx("modules:composer.upload-picture")}}">
		...
		<li title="{{tx("modules:composer.upload-file")}}">
			<button data-format="upload" class="file-upload-btn btn btn-sm btn-link text-reset" aria-label="{{tx("modules:composer.upload-file")}}">
	...
	<div class="d-flex align-items-center gap-1">
		...
		<button class="btn btn-sm btn-link py-2 text-body fw-semibold text-nowrap" data-action="preview">
			<i class="fa fa-eye"></i>
			<span class="d-none d-md-inline show-text">{{tx("modules:composer.show-preview")}}</span>
			<span class="d-none d-md-inline hide-text">{{tx("modules:composer.hide-preview")}}</span>
		</button>
		{{{ if composer:showHelpTab }}}
		<button class="btn btn-sm btn-link py-2 text-body fw-semibold text-nowrap" data-action="help">
			<i class="fa fa-question"></i>
			<span class="d-none d-md-inline">{{tx("modules:composer.help")}}</span>
		</button>
		{{{ end }}}
	</div>
</div>
